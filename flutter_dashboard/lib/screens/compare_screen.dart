    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            Tab(text: 'PnL Comparison'),
            Tab(text: 'Volume Comparison'),
          ],
        ),
      ),
      body: Column(
    ...
    return compareAsync.when(
      ...
      error: (e, _) => Center(
        child: Padding(
          ...
          child: Text('Could not load comparison data:\n$e', style: const TextStyle(color: Colors.redAccent), textAlign: TextAlign.center),
        ),
      ),
      data: (data) {
        if (data.isEmpty) {
          return const Center(child: Text('No data returned for selected symbols.', style: TextStyle(color: Colors.white38)));
        }
        return TabBarView(
